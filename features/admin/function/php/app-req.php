...
// Update the SQL query to select only the required fields
...
function submitDecline() {
    ...
    var id = document.getElementById('modalUserId').value; // Assuming 'modalUserId' is the correct identifier

    // Set the action to delete the appointment
    form.action = '../../function/php/delete_appointment.php'; // Set the action for decline
    form.method = 'POST';

    // Add the ID to the form as a hidden input
    var idInput = document.createElement("input");
    idInput.type = "hidden";
    idInput.name = "id"; // Ensure this matches the expected POST key in your PHP script
    idInput.value = id;
...
// Populate the user_id when the modal is opened
$('#viewModal').on('show.bs.modal', function (event) {
    var button = $(event.relatedTarget);
    var id = button.data('id'); // Assuming you have the ID in your button data attributes
    $('#modalUserId').val(id); // Set the user ID in the hidden input
    ...
    var additionalInfo = button.data('add-info'); // Get the add_info value
    ...
    modal.find('#add-info').val(additionalInfo); // Populate the street name input
...
$('#viewModal').on('show.bs.modal', function (event) {
    ...
    function initMap(lat, lng) {
        var mapOptions = {
            center: new google.maps.LatLng(lat, lng), // Center the map on the given coordinates
            zoom: 18, // Zoom level
    ...
    modal.find('#declineButton').off('click').on('click', function() {
        $.ajax({
            url: '../../function/php/delete_user.php', // The URL to your delete handler
            type: 'POST',
            data: { user_id: id },
